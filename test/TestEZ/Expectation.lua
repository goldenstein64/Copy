...
function Expectation.new(value)
	...
	self.rawEqual = bindSelf(self, self.rawEqual)
...
function Expectation:rawEqual(otherValue)
	local result = rawequal(self.value, otherValue) == self.successCondition

	local message = formatMessage(self.successCondition,
		("Expected raw value %q (%s), got %q (%s) instead"):format(
			tostring(otherValue),
			type(otherValue),
			tostring(self.value),
			type(self.value)
		),
		("Expected anything but raw value %q (%s)"):format(
			tostring(otherValue),
			type(otherValue)
		)
	)
